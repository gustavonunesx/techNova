...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let state = {
        produto: {
            id: editando ? parseInt(produtoId) : Date.now(), // ID único
            nome: '',
            textoDescritivo: '',
            cor: '',
            fabricante: '',
        ...
        produtosCadastrados: JSON.parse(localStorage.getItem('produtos')) || [],
        mostrarLista: false
    };

    // Se for edição, carregar o produto existente
    if (editando) {
        const produtoExistente = state.produtosCadastrados.find(p => p.id == produtoId);
        if (produtoExistente) {
            state.produto = {...produtoExistente};
        }
    }

    // Função para salvar produtos no localStorage
    function salvarProdutos() {
        localStorage.setItem('produtos', JSON.stringify(state.produtosCadastrados));
    }
    ...
    function handleSubmit(e) {
        ...
        // Validações
        if (!state.produto.nome || !state.produto.textoDescritivo || !state.produto.cor || 
            !state.produto.fabricante || !state.produto.preco || !state.produto.quantidade) {
        ...
        if (editando) {
            ...
            const index = state.produtosCadastrados.findIndex(p => p.id == state.produto.id);
            if (index !== -1) {
                state.produtosCadastrados[index] = novoProduto;
            }
        } else {
            // Adicionar novo produto
            state.produtosCadastrados.push(novoProduto);
        }
    ...
    function render() {
        root.innerHTML = `
                ...
                <div class="container">
                    <div class="header-content">
                        <div class="logo">
                            ...
                            <h1>Tech<span>Nova</span> - ${editando ? 'Editar' : 'Cadastrar'} Produto</h1>
                        </div>
            ...
            <div class="app">
                <h1>TechNova - ${editando ? 'Editar Produto' : 'Cadastro de Produtos'}</h1>
                ...
                <form id="produtoForm" class="form-container">
                    ...
                    <button type="submit" class="submit-btn">${editando ? 'Atualizar' : 'Cadastrar'} Produto</button>
                </form>
